...
class TencentNLP:
    ...
    '''-----------情感分析--------------'''
    def testSentiment(self,content,type=4):
        ...
        if result['message']=='':
            ...
            positive=float(Decimal.from_float(positive_ori).quantize(Decimal('0.00')))
            negative_ori=1-positive
            negative=round(negative_ori,2)
            validComment={"positive":positive,"negative":negative,"code":result["code"],'codeDesc':result['codeDesc']}
            return validComment
        else:
    ...
    '''-----------关键词提取------------'''
    def testKeywords(self,title,content,channle='CHnews_news_cul'):
        action = 'TextKeywords'
        params = {
             "title" : title,
             "content" : content
